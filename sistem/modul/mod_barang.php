...
switch($_GET[act]){
  ...
  default:  // ========================================================================================================================
	...
	<div style='height:90px;width:100%'>	
		      <form method=POST action='?module=barang&act=tambahbarang' style='float:left'>
              <input type=submit accesskey='t' value='(t) Tambah Barang'/>
              </form>

			  <form method=POST action='?module=barang&act=caribarang1' style='float:left'>
			  <input type=submit accesskey='c' value='(c) Cari Barang'/>
			  </form>
	
			  <form method=POST action='?module=barang&act=cetaklabel1' style='float:left'>
			  <input type=submit accesskey='l' value='(l) Cetak Label'/>
			  </form>

			  <form method=POST action='?module=barang&act=cetakbarang1' style='float:left'>
			  <input type=submit accesskey='b' value='(b) Cetak Stock Barang'/>
			  </form>

			  <form method=POST action='?module=barang&act=cetakSO' style='float:left'>
			  <input type=submit accesskey='s' value='(s) Cetak Stock Opname' />
			  </form>

			  <form method=POST action='?module=barang&act=inputSO' style='float:left'>
			  <input type=submit accesskey='o' value='(o) Input Stock Opname'/ >
			  </form>

			  <form method=POST action='?module=barang&act=inputrak' onSubmit=\"popupform(this, 'inputrak')\"  style='float:left'>
			  <input type=submit accesskey='i' value='(i) Input Cepat Rak' />
			  </form>

			  <form method=POST action='../tools/fast-stock-opname/fast-SO.php' style='float:left'>
			  <input type=submit value='Input Fast SO' />
			  </form>

			  <form method=POST action='?module=barang&act=ApproveFastSO1' style='float:left'>
			  <input type=submit value='Approve Fast SO'/ >
			  </form>

			  <form method=POST action='?module=barang&act=returbarang1' onSubmit=\"popupform(this, 'INPUT_RETUR_BARANG')\"  style='float:left'>
	...
	<p>&nbsp;</p>
          ...
          <tr><th>no</th><th>Barcode</th><th>Nama Barang</th><th>Kategori Barang</th>
                <th>Satuan Barang</th><th>Harga Beli</th><th>Harga Jual</th><th>aksi</th></tr>";
	...
	// jumlah data yang akan ditampilkan per halaman
			...
			$sql='SELECT DISTINCT(COUNT(*)) as jumData FROM barang';
			$result  = mysql_query($sql);
			$data     = mysql_fetch_assoc($result);
			$jumData = $data['jumData'];
			if(!isset($_GET['act']) && !isset($_GET['id'])){
			$sql="SELECT 
`barang`.`idBarang`, `barang`.`namaBarang`, `kategori_barang`.`namaKategoriBarang`, `satuan_barang`.`namaSatuanBarang`,`barang`.`barcode`, `barang`.`qtyStock`, `barang`.`qtyMinimum`, `barang`.`qtyMaximum`, `barang`.`hargaBeli`,`barang`.`hargaJual` 
FROM `barang`
LEFT JOIN `kategori_barang`
ON `barang`.`idKategoriBarang` = `kategori_barang`.`idKategoriBarang`
LEFT JOIN `satuan_barang`
ON `barang`.`idSatuanBarang` = `satuan_barang`.`idSatuanBarang` LIMIT ".$offset.", ".$dataPerPage."";
				$tampil=mysql_query($sql);
				$cek=mysql_num_rows($tampil);
				$no=1;
				if($cek==0)
				{
					echo "<tr><td colspan='8'>Data Barang tidak ada!</td></tr>";
				}
				else
				{
					$no=1; $ctr=1;
					while (($r=mysql_fetch_array($tampil))){
						...
						echo "<tr bgcolor=$warna>";//end warna
						   echo "<td align=right class=td>$no</td>
								 ...
								 <td align=right class=td>".number_format($r[hargaBeli],0,',','.')."</td>
								 <td align=right class=td>".number_format($r[hargaJual],0,',','.')."</td>
								 <td class=td><a href=?module=barang&act=editbarang&id=$r[barcode]>Ubah</a> | <a href=./aksi.php?module=barang&act=hapus&id=$r[idBarang]>Hapus</a>
								 </td></tr>";
						  $no++; $ctr++;
					}
				}
				...
				echo "Pages (".$jumPage.") : ";
				if ($noPage > 1) echo  "<a class='page' href='?module=barang&page=".($noPage-1)."'>&lt;Prev</a>";
				...
				for($page = 1; $page <= $jumPage; $page++)
				{
					...
					{   
						...
						if ($page == $noPage)
							...
							echo " <a class='page' href='?module=barang&page=".$page."'>".$page."</a> ";
						$showPage = $page;          
					}
				}
				// menampilkan link next
				if ($noPage < $jumPage) echo "<a class='page' href='?module=barang&page=".($noPage+1)."'>Next&gt;</a>";
				echo "</div>
				</table>
				<p>&nbsp;</p>
  ...
  case "tambahbarang": // ========================================================================================================================
    echo "<h2>Tambah Barang</h2>
          ...
          <table>
          <tr><td>Barcode</td><td> : <input type=text name='barcode' size=30 required ></td></tr>
          <tr><td>Nama Barang</td><td> : <input type=text name='namaBarang' size=30 required></td></tr>
          <tr><td>Supplier</td>
          ...
          <tr><td>Satuan Barang</td>
			...
			<tr><td>Jumlah Barang</td>
				<td> : <input type='number' name='qtyStock' /></td>
			</tr>
			<tr><td>Jumlah Minimum</td>
				<td> : <input type='number' name='qtyMin' required/></td>
			</tr>
			<tr><td>Jumlah Maksimum</td>
				<td> : <input type='number' name='qtyMax' required/></td>
			</tr>
          <tr><td colspan=2>&nbsp;</td></tr>
  ...
  case "editbarang": // ========================================================================================================================
    ...
    echo "<h2>Edit Barang</h2>
          ...
          <table>
          <tr><td>Barcode</td><td> : <input type=text name='barcode' size=30 value='$data[barcode]' readonly></td></tr>
		  ...
		  <tr><td>Supplier</td>
            ...
            echo "</select></td></tr>
			<tr><td>Harga Jual</td><td> : <input type=text name='hargaBeli' size=20 value='".$data[hargaBeli]."'></td></tr>
          	<tr><td>Harga Jual</td><td> : <input type=text name='hargaJual' size=20 value='".$data[hargaJual]."'></td></tr>
			<tr><td>Jumlah Barang</td>
				<td> : <input type='number' name='qtyStock' /></td>
			</tr>
		  	<tr><td>Jumlah Minimum</td>
				<td> : <input type='number' name='qtyMin' value='".$data[qtyMinimum]."'/></td>
			</tr>
			<tr><td>Jumlah Maksimum</td>
				<td> : <input type='number' name='qtyMax' value='".$data[qtyMaximum]."'/></td>
			</tr>
          	<tr><td colspan=2>&nbsp;</td></tr>
          	<tr><td colspan=2 align='right'><input type=submit value=Simpan>&nbsp;&nbsp;&nbsp;
                            <input type=button value=Batal onclick=self.history.back()></td></tr>
